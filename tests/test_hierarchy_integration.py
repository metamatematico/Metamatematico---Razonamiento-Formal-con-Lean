...
class TestCRTacGraphAware:
    ...
    def test_keyword_assist_still_works(self):
        """Lean-specific keywords trigger ASSIST; ambiguous words do not."""
        # Unambiguously Lean → ASSIST
        assert self.cr_tac.classify_query("use simp in lean") == ActionType.ASSIST
        assert self.cr_tac.classify_query("formaliza en lean 4") == ActionType.ASSIST
        assert self.cr_tac.classify_query("sorry rfl linarith") == ActionType.ASSIST
        # Ambiguous single words / common math prose → RESPONSE
        assert self.cr_tac.classify_query("prove this theorem") == ActionType.RESPONSE
        assert self.cr_tac.classify_query("ring theory basics") == ActionType.RESPONSE
        assert self.cr_tac.classify_query("apply the theorem") == ActionType.RESPONSE
    ...
    def test_graph_skill_with_tactics_returns_response(self):
        """Query matching a skill connected to tactics is still RESPONSE
        unless it explicitly requests Lean code (no false positives)."""
        # "ring theory question" is conversational even though ring-theory
        # connects to tactic-ring in the graph.
...
class TestIntegration:
    ...
    def test_cr_tac_feeds_context_pipeline(self):
        ...
        # 1. CR_tac classifies with graph — conversational → RESPONSE
        action = cr_tac.classify_query("ring theory question", graph=graph)
        assert action == ActionType.RESPONSE
