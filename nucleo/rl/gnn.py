...
def graph_to_pyg(graph: SkillCategory) -> Data:
    """
    Convertir SkillCategory a objeto PyG Data.

    Args:
        graph: Grafo categorico de skills

    Returns:
        Data con x, edge_index, edge_attr
    """
    skills = graph.skills
    ...
    # Mapeo skill_id -> indice de nodo
    skill_id_to_idx = {s.id: i for i, s in enumerate(skills)}

    # Calcular max level para normalizar
    ...
    for morphism in graph.morphisms:
        ...
        # Edge feature: morphism_type one-hot(5) + weight(1)
...
def _safe_log(value: int) -> float:
    """Log(1 + value) para normalizar grados."""
    import math
...
class SkillGNN(nn.Module):
    """
    GNN Encoder del grafo de skills.

    Usa GATConv con edge features para message passing con atencion.
    Produce un embedding de dimension hidden_dim para el grafo completo.
    """
    ...
    def forward(self, data: Data) -> torch.Tensor:
        ...
        Args:
            data: PyG Data con x, edge_index, edge_attr

        Returns:
            graph_embedding: [batch_size, hidden_dim]
        """
        if data.x.size(0) == 0:
            # Grafo vacio -> embedding cero
            batch_size = 1
            return torch.zeros(batch_size, self.hidden_dim, device=data.x.device)
