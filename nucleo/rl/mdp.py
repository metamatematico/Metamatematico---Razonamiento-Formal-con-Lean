...
class ExperienceBuffer:
    ...
    def save(self, path) -> None:
        """Persistir buffer a disco (pickle)."""
        import pickle
    ...
    @classmethod
    def load(cls, path, capacity: int = 10000) -> "ExperienceBuffer":
        """Cargar buffer desde disco; devuelve buffer vacío si no existe."""
        import pickle
        from pathlib import Path
        buf = cls(capacity)
        p = Path(path)
        ...
        try:
            ...
            for t in transitions[-capacity:]:
                buf.push(t)
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"No se pudo cargar buffer: {e}")
        return buf
