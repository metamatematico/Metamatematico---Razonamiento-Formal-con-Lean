...
es 38 de 39 (97\,\%). Todas las cifras
...
\toprule
 & Operaciones & \% \\
\midrule
Respaldadas por un teorema & 38 & 97 \\
Sin respaldo formal & 1 & 3 \\
Mapeo roto & 0 & 0 \\
\midrule
Total auditado & 39 & 100 \\
\bottomrule
\end{tabular}
\end{center}

\noindent Declaraciones formales disponibles en
\texttt{MetamathProver/CategoryFoundations/}: \textbf{197}.
...
\midrule \multicolumn{3}{l}{\textit{category.py}} \\
  ...
  & \texttt{classify\_link} & \texttt{simple\_of\_cluster} \\
  &  & \footnotesize simple = INDUCIDO por un cluster entre descomposiciones (Ehresmann); la lectura por factorizacion esta en composite\_of\_simple\_is\_simple \\
...
\midrule \multicolumn{3}{l}{\textit{patterns.py}} \\
  ...
  & \texttt{verify\_multiplicity\_principle} & \texttt{multiplicidad\_necesaria\_para\_complejidad} \\
  &  & \footnotesize sin Principio de Multiplicidad todo compuesto de simples es simple: MP es condicion NECESARIA de la complejidad, no un adorno sobre robustez \\
  & \texttt{detect\_pattern\_in\_graph} & \texttt{enlaces\_complejos\_existen} \\
  &  & \footnotesize modelo concreto: dos enlaces simples cuya composicion no es simple \\
  & \texttt{\_connected\_by\_cluster} & \texttt{complex\_needs\_unconnected} \\
  &  & \footnotesize un enlace complejo exige descomposiciones intermedias NO conectadas \\
\midrule \multicolumn{3}{l}{\textit{complexity.py}} \\
  & \texttt{\_detect\_convergence\_patterns} & \texttt{MP\_alcanzable\_en\_preorden} \\
  &  & \footnotesize emite tambien las descomposiciones por subconjunto: MP es alcanzable \\
  & \texttt{compute\_complexity\_order (multi)} & \texttt{hierarchy\_well\_founded\_multi} \\
  &  & \footnotesize con varias descomposiciones, el MAXIMO converge; asignar oscilaba \\
\midrule \multicolumn{3}{l}{\textit{patterns.py}} \\
  & \texttt{son\_homologos} & \texttt{Homologos} \\
  &  & \footnotesize homologia de Ehresmann: mismo colimite \\
...
\midrule \multicolumn{3}{l}{\textit{evolution.py}} \\
  & \texttt{TransitionFunctor} & \texttt{comp} \\
  &  & \footnotesize funtor parcial entre instantaneas: none = objeto eliminado \\
  & \texttt{compose} & \texttt{comp\_assoc} \\
  &  & \footnotesize la composicion de transiciones es asociativa \\
  & \texttt{verify\_compatibility} & \texttt{compatible\_iff} \\
  &  & \footnotesize compatibilidad (Def 3.2) = ley de funtor sobre el orden del tiempo \\
  & \texttt{apply\_option} & \texttt{eliminado\_es\_absorbente} \\
  &  & \footnotesize lo eliminado no vuelve: ninguna composicion posterior lo recupera \\
  & \texttt{measure\_emergence} & \texttt{soloCrece\_cadena} \\
  &  & \footnotesize emergence\_ratio es comparable entre tiempos si nada se elimina \\
  & \texttt{detect\_complex\_links} & \texttt{complex\_needs\_unconnected} \\
  &  & \footnotesize un enlace complejo exige descomposiciones intermedias no conectadas \\
\bottomrule
\end{longtable}
...
\midrule
\texttt{patterns.py} & \texttt{campos\_operativos\_isomorfos} & \footnotesize heuristica estructural, NO la homologia de Ehresmann \\
\bottomrule
\end{tabular}
\end{center}
...
\item \texttt{MetamathProver/CategoryFoundations/} — 197 declaraciones.
\end{itemize}
